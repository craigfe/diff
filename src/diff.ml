...
let construct_grid (type a) ~(equal : a -> a -> bool) (a : a Subarray.t)
    (b : a Subarray.t) : int Array.Matrix.t =
  let grid_x_length = Subarray.length a + 1
  and grid_y_length = Subarray.length b + 1 in
  let grid = Array.Matrix.make grid_x_length grid_y_length 0 in
  let get_grid = Array.Matrix.get grid in
    ...
    for j = 0 to grid_y_length - 1 do
      let cost =
        if min i j = 0 then max i j
          ...
          ((i - 1, j), (i, j - 1), (i - 1, j - 1))
          |> Triple.map get_grid
          |> Triple.minimum ~ord:Int.ord
          |> ( + ) 1
      in
      Array.Matrix.set grid (i, j) cost
    done
  done;
  grid
...
let reconstruct_edit_script a b grid =
  ...
  let backtrack (i, j) =
    ...
    if Subarray.get a (i - 1) = Subarray.get b (j - 1) then (p_sub, [])
      ...
      |> Triple.minimum_on ~ord:Int.ord snd
      |> function
    ...
    | pos ->
        ...
        Logs.debug (fun f ->
            f "%a: %a"
              ...
              pos (Fmt.Dump.list pp_command) action);
        (aux [@tailcall]) (action @ acc) next_coord
  in
  let x_len, y_len = Array.Matrix.dimensions grid in
  aux [] (x_len - 1, y_len - 1)
